...
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
  using read_csv(kings2)is the correct function and gives the best sheet
  
2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
  running class() it gives us a tibble: "spec_tbl_df" "tbl_df" "tbl"     "data.frame"
  
3. How many columns does the object have when created with these different functions?
  when using read.csv we get 11 columns, read_csv gives 11 columns, while read.csv2 and read_csv2 gives only 1 column
  
4. Show the dataset so that we can see how R interprets each column
  by using the function head() it inserts a table in our script
...
mean(kings_new$kings_duration,na.rm=TRUE)
kings_new %>% 
  summarize(mean_kings_duration=mean(kings_duration))
```
...
kings_new %>% 
  filter(kings_duration>mean(kings_new$kings_duration)) %>% 
  count(kings_duration>mean(kings_new$kings_duration))
```
...
longest %>% 
  mutate(Rule_days=kings_duration*365)
```
