...
date: ' created 17 march 2026, updated 20 march 2026'
output:
...
```{r seperate_country, eval = True}
#... YOUR CODE
Western_Europe %>% 
  ggplot(aes(x=Year, y=homicide_per_100k, color=country))+
  geom_line(stat = "identity", position = "dodge")+
  facet_wrap(~country)+
  labs(title = "Homicides Per 100k in Western Europe", x="year", y="homicide_per_100k")+
...
- How would you characterize the trend compared to the homicides above?
    The homicide graphs are mostly a smooth developemnt going down, while the     danish kings reign, is eatic and all over the place

```{r kings_graph, eval = TRUE}
# YOUR CODE HERE:
kings <- read_csv2("data/Dam_Kongehus_Clean.csv")

kings %>% 
  mutate(Midyear=Regeringtids_Slut - Antal_regeringsår) %>% 
  ggplot(aes(x=Midyear, y=Antal_regeringsår))+
